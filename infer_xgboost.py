# /home/liyiming/math/infer_xgboost.py
...
class SimpleLabelEncoder:
    ...
    def transform(self, y):
        """转换数据，未知类别映射为-1"""
        return np.array([self.class_to_index.get(str(val), -1) for val in y])
...
def encode_categorical_features(df: pd.DataFrame, cat_feats: list, label_encoders: dict) -> pd.DataFrame:
    """使用保存的标签编码器对分类特征进行编码"""
    df_encoded = df.copy()
    
    for col in cat_feats:
        if col in df_encoded.columns and col in label_encoders:
            encoder = SimpleLabelEncoder(label_encoders[col])
...
# ------------------ 主流程：仅推理并输出提交 ------------------
def main():
    model_path = "/home/liyiming/math/model.xgb"
    meta_path = "/home/liyiming/math/model_meta_xgboost.json"
    out_path = "/home/liyiming/math/submission_xgboost.csv"

    meta = load_meta(meta_path)
    base_dir = meta.get("base_dir", "/home/liyiming/math")
    train_cols = meta["train_cols"]
    cat_feats = meta.get("cat_feats", [])
    label_encoders = meta.get("label_encoders", {})
    model_type = meta.get("model_type", "lightgbm")
    
    model = load_model(model_path)

    print("=== XGBoost Inference (rolling 28d) ===")
    print(f"base_dir={base_dir}")
    print(f"model_type={model_type}")
    print(f"FDAY={FDAY.date()}, H={H}, MAX_LAGS={MAX_LAGS}")
    print(f"#features={len(train_cols)}: {train_cols[:8]} ...")
    print(f"#categorical_features={len(cat_feats)}: {cat_feats}")
    print()
    ...
    for t in range(H):
        ...
        # 获取当天的特征数据
        X_t = wnd.loc[wnd.date == day, train_cols].copy()
        
        # 对分类特征进行编码
        if cat_feats and label_encoders:
            X_t_encoded = encode_categorical_features(X_t, cat_feats, label_encoders)
        else:
            X_t_encoded = X_t
        
        # 预测
        predictions = model.predict(X_t_encoded)
